...
public class SkeletonServicesRpcProxy implements ISkeletonServices {
    ...
    public Move[] findAllMovesPlayer(String username) throws SkeletonException {
        System.out.println("PROXY - FIND CURRENT MOVES PLAYER");
        GameData data = new GameData(username, null);
        Request req=new Request.Builder().type(RequestType.FIND_CURRENT_MOVE_PLAYER).data(data).build();
        sendRequest(req);
        Response response=readResponse();
        if (response.type()== ResponseType.ERROR){
            String err=response.data().toString();
