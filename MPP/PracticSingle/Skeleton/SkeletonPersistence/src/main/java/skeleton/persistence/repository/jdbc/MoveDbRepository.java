...
public class MoveDbRepository implements IMoveRepository {
    ...
    public void save(Move entity) {
        try(Session session = sessionFactory.openSession()) {
            ...
            try {
                ...
                System.err.println("Eroare la save Move: " + ex);
                if (tx != null)
                    tx.rollback();
            }
        }
    ...
    @Override
    public Iterable<Move> findAllPlayer(String username) {
        try(Session session = sessionFactory.openSession()) {
            Transaction tx = null;
            try {
                tx = session.beginTransaction();

                Iterable<Move> moves = session.createQuery("from Move where username = :username", Move.class)
                        .setParameter("username",  username)
                        .list();
    ...
    public void deleteForUser(String username) {
        try(Session session = sessionFactory.openSession()) {
            ...
            try {
                ...
                session.createQuery("delete from Move where username = :username")
                        .setParameter("username", username)
                        .executeUpdate();
